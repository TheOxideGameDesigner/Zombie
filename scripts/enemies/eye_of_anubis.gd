...
const HIT_TIME : float = 5.0


@onready var orb = $orb
@onready var ray = $ray
@onready var init_ray_pos : Vector3 = $ray.position
...
@onready var init_orb_pos : float = orb.position.y
@onready var init_death_ray_pos : float = death_ray.position.y
...
func _process(delta : float) -> void:
	...
	var dir : Vector3 = player_cam.global_position - orb.global_position
	...
	if not sees_player:
		...
		return
	hit_timer += delta
	var opac : float = hit_timer / HIT_TIME
