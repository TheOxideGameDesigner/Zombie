...
var target_diff = 2
@onready var diff = get_parent().min_dif
@export var radius = 0.5
@export var height = 2.0

var mesh
const COLORS = [Color(0.5, 1, 0, 0.2), Color(0, 0.7, 0, 0.3), Color(1.0, 1.0, 0, 0.5), Color(1.0, 0.3, 0.0, 0.3), Color(0.3, 0.0, 0.3, 0.4)]
...
func _ready():
	if Engine.is_editor_hint():
		...
		mesh.material_override.albedo_color = COLORS[diff]
...
func _process(delta):
	...
	if pressed_minus and not prev_pressed_minus:
		...
		mesh.get_parent().get_parent().visible = diff <= target_diff
